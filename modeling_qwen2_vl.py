...
class Qwen2VLTextModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        pruning_layers: list[int] | None = None,
        pruning_ratios: dict[int, float] | None = None,
        mm_token_type_ids: torch.IntTensor | None = None,
        prune_gaze: bool | None = None,
        prune_text: bool | None = None,
        prune_alpha: float | None = None,
        gaze_scores: torch.FloatTensor | None = None,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        ## ---- PRUNING SETUP ----
        pruning_layers = [] if pruning_layers is None else pruning_layers
        pruning_ratios = {} if pruning_ratios is None else pruning_ratios

        original_visual_token_num = None
        is_prefill = hidden_states.shape[1] !=1 and (past_key_values is None or past_key_values.get_seq_length() == 0)

        for i, decoder_layer in enumerate(self.layers):
            ## ---- PRUNING LAYER ----
            if i in pruning_layers and is_prefill and mm_token_type_ids is not None and (prune_text or prune_gaze):
                ...
                ## ---- SPARSE VLM ----
                # calculate text raters and sparsevlm operations here
                keep_ratio = pruning_ratios[i]
                # 1. Identify visual/text token positions in current sequence
                visual_pos, text_pos = self.build_modality_positions(mm_token_type_ids)

                # Save original count once, to match SparseVLM behavior
                if original_visual_token_num is None:
                    original_visual_token_num = int(visual_pos.numel())

                # Initialise optional variables so debug storage is always safe
                text_rater_scores = None
                rater_mask = None
                text_visual_scores = None
                gaze_visual_scores = None

                if prune_text:
                    # 2. SparseVLM text-rater selection from current hidden states
                    text_rater_scores, selected_text_pos, rater_mask = self.select_text_raters_sparsevlm(
                        hidden_states,
                        visual_pos,
                        text_pos,
                    )

                    # 3. SparseVLM visual scores from current layer attention
                    text_visual_scores = self.compute_sparsevlm_visual_scores(
                        attn_weights,
                        visual_pos,
                        selected_text_pos,
                    )

                # 4. Normalise to [0, 1] so prune_alpha is meaningful across both signals
                if prune_text:
                    text_visual_scores = self._minmax(text_visual_scores)

                if prune_gaze and gaze_scores is not None:
                    # gaze_scores is (Lv,) pre-aligned to visual tokens in sequence order
                    gaze_visual_scores = self._minmax(gaze_scores)
                elif prune_gaze:
                    # prune_gaze requested but no tensor passed — degrade gracefully to text-only
                    prune_gaze = False

                if prune_gaze and prune_text:
                    combined_visual_scores = prune_alpha * text_visual_scores + (1 - prune_alpha) * gaze_visual_scores
                elif prune_gaze:
                    combined_visual_scores = gaze_visual_scores
                elif prune_text:
                    combined_visual_scores = text_visual_scores
                else:
                    combined_visual_scores = None
                    
                # 5. Build visual keep mask
                visual_keep_mask = self.build_sparsevlm_visual_keep_mask(
                    combined_visual_scores,
                    keep_ratio=keep_ratio,
                    original_visual_token_num=original_visual_token_num,
                )

                # 6. Expand to full sequence keep mask
                full_keep_mask = self.build_full_sequence_keep_mask(
                ...
                # 7. Prune all aligned tensors together
                (
                    hidden_states,
                    position_ids,
                    text_position_ids,
                    ...
                    rotary_emb_module=self.rotary_emb,
                )

                # Store pruning state for external visualization / debugging.
                # Stored on self so the caller can read it after generate() returns.
                # Keyed by layer index so multi-layer pruning is supported.
                _dbg: dict = {
                    "visual_keep_mask":    visual_keep_mask.detach().cpu(),
                    "combined_visual_scores": combined_visual_scores.detach().cpu()
                        if combined_visual_scores is not None else None,
                    "visual_pos": visual_pos.detach().cpu(),
                    "text_pos":   text_pos.detach().cpu(),
                }
                if text_rater_scores is not None:
                    _dbg["text_rater_scores"] = text_rater_scores.detach().cpu()
                if rater_mask is not None:
                    _dbg["rater_mask"] = rater_mask.detach().cpu()
                if text_visual_scores is not None:
                    _dbg["text_visual_scores"] = text_visual_scores.detach().cpu()
                if gaze_visual_scores is not None:
                    _dbg["gaze_visual_scores"] = gaze_visual_scores.detach().cpu()
                if not hasattr(self, "_pruning_debug"):
                    self._pruning_debug = {}
                self._pruning_debug[i] = _dbg

                # Slice gaze_scores to stay aligned with surviving visual tokens.
                # Required when multiple pruning layers are specified: the next
                # pruning layer will see a shorter visual_pos, so gaze_scores must
                # match that length exactly.
                if gaze_scores is not None:
                    gaze_scores = gaze_scores[visual_keep_mask]

                # Optional debug
                print(
                    f"[layer {i}] kept {int(visual_keep_mask.sum().item())}/{int(visual_pos.numel())} visual tokens"
                )

            ## ---- NO PRUNING ----
            else:
                # During decode, layers that come *after* any pruning layer have a
                # shorter KV cache (pruned-length) than the top-level causal_mask,
                # which was built against the full-prefill KV cache at layer 0.
                # Passing that mask causes a shape mismatch:
                #   attn_weights  dim-3 = pruned_len + decode_step
                #   causal_mask   dim-3 = full_prefill_len + decode_step
                # Fix: pass attention_mask=None for those layers during decode.
                # None is safe for single-token greedy/beam decode (no future tokens,
                # no padding, batch_size=1 — causal order is trivially satisfied).
                if not is_prefill and pruning_layers and i > max(pruning_layers):
                    layer_attn_mask = None
                else:
                    layer_attn_mask = causal_mask_mapping[self.config.layer_types[i]]
    ...
    def _minmax(t: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """Min-max normalise a 1-D score tensor to [0, 1]."""
        return (t - t.min()) / (t.max() - t.min() + eps)

    def build_modality_positions(self, mm_token_type_ids: torch.Tensor):
        """
        Assumes batch size 1 for the first version.
        text = 0, image = 1, video = 2
        """
        if mm_token_type_ids is None:
            raise ValueError("Need mm_token_type_ids for SparseVLM pruning.")

        if mm_token_type_ids.shape[0] != 1:
            raise NotImplementedError("First implementation assumes batch size 1.")

        token_types = mm_token_type_ids[0]  # (L,)
        seq_pos = torch.arange(token_types.shape[0], device=token_types.device)

        visual_pos = seq_pos[token_types == 2]
        text_pos = seq_pos[token_types == 0]

        # Optional: only use text after the visual block, matching your earlier logic
        if visual_pos.numel() > 0:
            text_pos = text_pos[text_pos > visual_pos[-1]]

        if visual_pos.numel() == 0:
            raise ValueError("No visual tokens found.")
        if text_pos.numel() == 0:
            raise ValueError("No text tokens found after visual block.")

        return visual_pos, text_pos
    
    def select_text_raters_sparsevlm(self, hidden_states, visual_pos, text_pos):
        """
        hidden_states: (B, L, D)
        returns:
        text_rater_scores: (Lt,)
        selected_text_pos: (Lr,)
        rater_mask: (Lt,)
        """
        v_t = hidden_states[:, visual_pos, :]   # (B, Lv, D)
        t_t = hidden_states[:, text_pos, :]     # (B, Lt, D)

        m_v_t = torch.matmul(v_t, t_t.transpose(1, 2))  # (B, Lv, Lt)
        m_v_t = torch.softmax(m_v_t, dim=2).mean(dim=1) # (B, Lt)
        ...
        text_rater_scores = m_v_t.squeeze(0)            # (Lt,)
        selected_text_pos = text_pos[rater_mask[0]]     # (Lr,)

        return text_rater_scores, selected_text_pos, rater_mask.squeeze(0)
    
    def compute_sparsevlm_visual_scores(self, attn_weights, visual_pos, selected_text_pos):
        """
        attn_weights: (B, H, Lq, Lk)
        returns:
        relation_vis_text: (Lv,)
        """
        attn_mean = attn_weights.mean(dim=1)  # (B, L, L)

        relation_vis_text = (
            attn_mean[:, selected_text_pos, :][:, :, visual_pos]
            .mean(dim=1)
            .squeeze(0)
        )  # (Lv,)
    ...
    def build_sparsevlm_visual_keep_mask(
        self, 
        visual_scores: torch.Tensor,
        keep_ratio: float,
        original_visual_token_num: int,
    ):
        """
        visual_scores: (Lv,)
        returns:
        visual_keep_mask: (Lv,) bool
        """
        v_token_num = visual_scores.numel()
        ratio_token_to_keep = int(original_visual_token_num * keep_ratio)

        visual_keep_mask = torch.zeros_like(visual_scores, dtype=torch.bool)

        if v_token_num != 0:
            k = min(ratio_token_to_keep, v_token_num)
            k = max(k, 1)  # always keep at least 1 visual token
            _, indices = torch.topk(visual_scores, k, dim=0)
            visual_keep_mask[indices] = True
        else:
            visual_keep_mask = torch.ones_like(visual_scores, dtype=torch.bool)

        return visual_keep_mask


    def build_full_sequence_keep_mask(self, seq_len, visual_pos, visual_keep_mask, device):
        """
        Converts visual-only keep mask into full-sequence keep mask.
        All non-visual tokens are kept.
        """
        full_keep_mask = torch.ones(seq_len, dtype=torch.bool, device=device)
        full_keep_mask[visual_pos] = visual_keep_mask
        return full_keep_mask


    def slice_causal_mask_mapping(self, causal_mask_mapping, keep_idx):
        """
        Slices all prepared masks after pruning.
        Assumes masks are either None or (..., Q, K).
        """
        new_mapping = {}
        for k, mask in causal_mask_mapping.items():
            if mask is None:
                new_mapping[k] = None
            else:
                new_mapping[k] = mask.index_select(-2, keep_idx).index_select(-1, keep_idx)
        return new_mapping


    def prune_sequence_state(
        self,
        hidden_states,
        position_ids,
        text_position_ids,
        ...
        """
        Applies the same keep mask to every sequence-aligned tensor.
        """
        keep_idx = full_keep_mask.nonzero(as_tuple=True)[0]

        hidden_states = hidden_states[:, keep_idx, :]              # (B, Lnew, D)
        position_ids = position_ids[:, :, keep_idx]                # (3, B, Lnew)

        if text_position_ids is not None:
            text_position_ids = text_position_ids[:, keep_idx]     # (B, Lnew)

        if mm_token_type_ids is not None:
            mm_token_type_ids = mm_token_type_ids[:, keep_idx]     # (B, Lnew)

        causal_mask_mapping = self.slice_causal_mask_mapping(causal_mask_mapping, keep_idx)

        # Recompute rotary embeddings for the shorter sequence
        position_embeddings = rotary_emb_module(hidden_states, position_ids)
