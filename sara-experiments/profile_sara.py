"""
profile_sara.py — Extended profiling script for Sara's experiments.

Changes vs profile_forked_model.py:
  - Paths resolve to main repo root (works from inside the worktree).
  - CSV has two new columns:
      cot_output        : full decoder text (reasoning + answer) for MCQ questions
      llm_judge_score   : filled offline by llm_judge.py
      llm_judge_reasoning: filled offline by llm_judge.py
  - Caption task is unchanged (BERTScore offline via score_captions_sara.py).
"""
...
FORKED_MODEL_PATH = REPO_ROOT / "modeling_qwen2_vl.py"
LOG_FILE = Path(__file__).parent / "profile_sara_log.md"
...
def run_sample(
    ...
    if task == "caption":
        ...
        is_correct       = None
        cot_output       = ""  # caption uses predicted_answer; BERTScore handles evaluation
    else:
        ...
        cot_output       = decoder_output.strip().replace("\n", " ")  # full reasoning chain
...
> {csv_row['_decoder_output'].replace(chr(10), ' ')[:800]}
...
"""
    ...
    with open(LOG_FILE, "a") as f:
        if run_number == 1:
            f.write("# Sara Experiments — Profiling Log\n")
        f.write(entry)
...
def print_batch_summary(config_tag: str, rows: list[dict]):
    ...
    if answered:
        ...
        print(f"  Accuracy: n/a (caption mode — score offline with score_captions_sara.py)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sara experiments: Qwen2-VL with CoT + LLM judge.")
    ...
    parser.add_argument("--prune-text",   action="store_true")
    parser.add_argument("--prune-gaze",   action="store_true")
    parser.add_argument("--prune-random", action="store_true")
